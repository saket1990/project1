...
const findBlogs = async function (req, res) {
    try {
        ...
        if (!allBlogs) return res.status(404).send({ status: false, msg: "No such Blogs are present" })
...
const filterBlogs = async function (req, res) {
    try {
        ...
        if (!gotBlogs) return res.status(404).send({ status: false, msg: "no such blogs are present" })
